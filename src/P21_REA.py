...
def rot_matr(centers, angle):
    ...
    for i, center in enumerate(centers):
        ...
        T = np.array([[1, 0, 0, -x], [0, 1, 0, -y], [0, 0, 1, 0], [0, 0, 0, 1]]).T
        R = np.array(
            [
        ...
        T_1 = np.array([[1, 0, 0, x], [0, 1, 0, y], [0, 0, 1, 0], [0, 0, 0, 1]]).T
...
def centers(
    start_point: np.ndarray, stop_point: np.ndarray, r: float, n_sides: float
) -> np.ndarray:
    ...
    centers_arr = np.hstack(
        (xv.reshape(-1, 1), yv.reshape(-1, 1), zv.reshape(-1, 1), ones.reshape(-1, 1))
    )
    return centers_arr
...
if __name__ == "__main__":
    ...
    for l in np.linspace(1, 26, 26):
        ...
        print(
            f"Calculating on polygon of n={n_sides}, length={l} and area={area_target}"
        )
        ...
        points = np.einsum(
            "kmn, in->ikm", trans_matrix, centers_array, optimize="optimal"
        )[:, :, :-1]
    ...
    for value in unique_scan_area_size:
        IQR_first = P21_all_data.loc[
            P21_all_data["scan_area_size"] == value, "P21"
        ].quantile(0.75) - P21_all_data.loc[
            P21_all_data["scan_area_size"] == value, "P21"
        ].quantile(
            0.25
        )
        IQR_second = P21_all_data.loc[
            P21_all_data["scan_area_size"] == value + 1, "P21"
        ].quantile(0.75) - P21_all_data.loc[
            P21_all_data["scan_area_size"] == value + 1, "P21"
        ].quantile(
            0.25
        ...
        IQR_results.append(
            {"scan_area_size": value + 0.5, "delta_IQR": delta_IQR}
        )  # sommo 0.5 alla scan area size per posizionare i valori di delta IQR a metà tra i boxplot, questo va bene per il passo di campionamento che utilizziamo adesso
    ...
    for value in unique_scan_area_size:
        IQR = P21_all_data.loc[P21_all_data["scan_area_size"] == value, "P21"].quantile(
            0.75
        ) - P21_all_data.loc[P21_all_data["scan_area_size"] == value, "P21"].quantile(
            0.25
        )
        lower_wisker = (
            P21_all_data.loc[P21_all_data["scan_area_size"] == value, "P21"].quantile(
    ...
    y_label = np.full(
        len(P21_all_data["scan_area_size"].unique()), -0.15
    )  # absolute frequency value position
    label = [
        f"{x}" for x in P21_all_data.groupby("scan_area_size").count()["P21"].to_list()
    ]

    # create box plots, delta IQR polt and delta wiskers plot
    fig = plt.figure(figsize=(8, 12))  # Overall figure size
    gs = gridspec.GridSpec(
        3, 1, height_ratios=[2.5, 1, 1]
    )  # set relative dimension of the three plots
    ...
    # ax1.grid(True)
    ax1.axhline(
        y=0.715, color="r", linestyle="--", label="Fixed Line (y=10)"
    )  # Outcrop mean p21, outcrop area is equivalent to an hexagon with a 65.42m side
    plt.suptitle("")
    ...
    wisker_results.plot(
        x="scan_area_size", y="delta_wiskers", kind="line", ax=ax3, color="red"
    )
